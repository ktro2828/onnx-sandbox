  ...
  const int * index_pair, const float * attn_weight, const float * value_features, float * output)
{
  ...
  if (query_idx >= total_query_num || head_idx >= nhead || hdim_idx >= hdim) return;
  ...
  const int * index_pair, const float * attn_weight, const float * value_features, float * output)
{
  ...
  switch (local_size) {
    case 16:
      attention_value_computation_forward<16><<<blocks, threads>>>(
      ...
      break;
    case 32:
      attention_value_computation_forward<32><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, output);
      break;
    case 64:
      attention_value_computation_forward<64><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, output);
      break;
    case 128:
      attention_value_computation_forward<128><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, output);
      break;
    case 320:
      attention_value_computation_forward<320><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, output);
      break;
    case 384:
      attention_value_computation_forward<384><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, output);
      break;
    default:
      attention_value_computation_forward<512><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, output);
      break;
  }
...
__global__ void attention_value_computation_backward(
  int b, int total_query_num, int local_size, int total_key_num, int nhead, int hdim,
  const int * query_batch_cnt, const int * key_batch_cnt, const int * index_pair_batch,
  const int * index_pair, const float * attn_weight, const float * value_features, float * grad_out,
  float * grad_attn_weight, float * grad_value_features)
{
  ...
  for (int i = hdim_idx; i < local_size; i += blockDim.x) {
    grad_attn_weight[query_idx * local_size * nhead + i * nhead + head_idx] =
      shared_grad_attn_weight[i];
  }
}
  ...
  float * grad_attn_weight, float * grad_value_features)
{
  ...
  switch (local_size) {
    case 16:
      attention_value_computation_backward<16><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, grad_out, grad_attn_weight,
        grad_value_features);
      break;
    case 32:
      attention_value_computation_backward<32><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, grad_out, grad_attn_weight,
        grad_value_features);
      break;
    case 64:
      attention_value_computation_backward<64><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, grad_out, grad_attn_weight,
        grad_value_features);
      break;
    case 128:
      attention_value_computation_backward<128><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, grad_out, grad_attn_weight,
        grad_value_features);
      break;
    case 320:
      attention_value_computation_backward<320><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, grad_out, grad_attn_weight,
        grad_value_features);
      break;
    case 384:
      attention_value_computation_backward<384><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, grad_out, grad_attn_weight,
        grad_value_features);
      break;
    default:
      attention_value_computation_backward<512><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, attn_weight, value_features, grad_out, grad_attn_weight,
        grad_value_features);
      break;
