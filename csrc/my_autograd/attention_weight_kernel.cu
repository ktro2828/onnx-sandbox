  ...
  const int * index_pair, const float * query_features, const float * key_features, float * output)
{
  ...
  if (query_idx >= total_query_num || head_idx >= nhead || local_key_idx >= local_size) return;
  ...
  const int * index_pair, const float * query_features, const float * key_features, float * output)
{
  ...
  switch (hdim) {  // switch hdim for utilizing different shared vectors.
    case 16:
      attention_weight_computation_forward<16><<<blocks, threads>>>(
      ...
      break;
    case 24:
      attention_weight_computation_forward<24><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, output);
      break;
    case 32:
      attention_weight_computation_forward<32><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, output);
      break;
    case 48:
      attention_weight_computation_forward<48><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, output);
      break;
    case 64:
      attention_weight_computation_forward<64><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, output);
      break;
    case 128:
      attention_weight_computation_forward<128><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, output);
      break;
    default:
      attention_weight_computation_forward<150><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, output);
      break;
  }
...
__global__ void attention_weight_computation_backward(
  int b, int total_query_num, int local_size, int total_key_num, int nhead, int hdim,
  const int * query_batch_cnt, const int * key_batch_cnt, const int * index_pair_batch,
  const int * index_pair, const float * query_features, const float * key_features,
  float * grad_out, float * grad_query_features, float * grad_key_features)
{
  ...
  if (index_pair[index] != -1) {
    ...
    for (int i = 0; i < hdim; i++) {
      atomicAdd(shared_grad_query_features + i, gradient * key_features[i]);
      atomicAdd(grad_key_features + i, gradient * shared_query_features[i]);
    }
  }
  __syncthreads();
  ...
  float * grad_out, float * grad_query_features, float * grad_key_features)
{
  ...
  switch (hdim) {
    case 16:
      attention_weight_computation_backward<16><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, grad_out, grad_query_features,
        grad_key_features);
      break;
    case 24:
      attention_weight_computation_backward<24><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, grad_out, grad_query_features,
        grad_key_features);
      break;
    case 32:
      attention_weight_computation_backward<32><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, grad_out, grad_query_features,
        grad_key_features);
      break;
    case 48:
      attention_weight_computation_backward<48><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, grad_out, grad_query_features,
        grad_key_features);
      break;
    case 64:
      attention_weight_computation_backward<64><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, grad_out, grad_query_features,
        grad_key_features);
      break;
    case 128:
      attention_weight_computation_backward<128><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, grad_out, grad_query_features,
        grad_key_features);
      break;
    default:
      attention_weight_computation_backward<150><<<blocks, threads>>>(
        b, total_query_num, local_size, total_key_num, nhead, hdim, query_batch_cnt, key_batch_cnt,
        index_pair_batch, index_pair, query_features, key_features, grad_out, grad_query_features,
        grad_key_features);
      break;
